...
var renderRent = function(rent){
  ...
  text += '\n';
  text += 'Požičané od: ' + rent.rent.startDate.toLocaleDateString();
  text += '\n';
  text += 'Požičané do: ' + rent.rent.endDate.toLocaleDateString();

  return text;
};

var renderReservation = function(rent){
  var text = '';
  text += 'Rezervovanie knihy ' + renderBookSubject(rent) + '.';
  text += '\n';
  text += '\n';
  text += 'Používateľ: ' + rent.user.name;
...
var renderReturn = function(rent){
  ...
  text += 'Požičané do: ' + rent.rent.endDate.toLocaleDateString();
  text += '\n';
  text += 'Vrátené: ' + rent.rent.returnDate.toLocaleDateString();
...
module.exports.reserveBook = function(rent){
  send(rent, 'reservation', 0);
};
